...
1. **启动服务**
   - 直接运行：访问 `http://localhost:5000`
   - Docker：访问 `http://localhost:6100`
...
1. **密码安全**：建议修改默认密码并定期更换
2. **文件权限**：确保Python有读写文件的权限
3. **端口占用**：确保5000端口（或自定义端口）未被其他程序占用
4. **网络访问**：0.0.0.0绑定允许外部访问，注意安全性
